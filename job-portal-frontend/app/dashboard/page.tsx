...
import { useAuth, useJobs, useJobStatistics } from '@/hooks/useApi'
...
export default function Dashboard() {
  ...
  const { statistics } = useJobStatistics()
  ...
  const stats = {
    ...
    activeJobs: jobs.filter(job => job.is_active && job.approved_by_admin).length,
    totalApplicants: jobs.reduce((sum, job) => sum + (job.application_count || 0), 0),
    newApplicants: 0, // This would need to come from applications API
    interviews: 0, // This would need to come from interviews API
    hires: 0, // This would need to come from hires API
    totalViews: jobs.reduce((sum, job) => sum + (job.view_count || 0), 0)
  }

  // Transform jobs data for charts
  const jobCategoryData = statistics?.jobs_by_category?.map((cat, index) => ({
    name: cat.category,
    value: Math.round((cat.count / (statistics.total_jobs || 1)) * 100),
    color: ['#FBBF24', '#3B82F6', '#10B981', '#F59E0B', '#6B7280'][index % 5]
  })) || []

  const applicationTrends = statistics?.monthly_stats?.map(stat => ({
    month: stat.month,
  ...
  const performanceMetrics = [
    { 
      metric: 'Active Jobs', 
      current: Math.round((stats.activeJobs / Math.max(stats.totalJobs, 1)) * 100), 
      target: 80, 
      color: '#10B981' 
    },
    { 
      metric: 'Job Views', 
      current: Math.min(Math.round(stats.totalViews / 10), 100), 
      target: 100, 
      color: '#3B82F6' 
    },
    { 
      metric: 'Application Rate', 
      current: Math.min(Math.round(stats.totalApplicants / Math.max(stats.totalViews, 1) * 100), 100), 
      target: 15, 
      color: '#F59E0B' 
    },
    { 
  ...
  const recentJobs = jobs.slice(0, 3).map(job => ({
    ...
    status: job.approved_by_admin ? 'Active' : 'Pending',
    postedAt: job.created_at ? new Date(job.created_at).toLocaleDateString() : 'Unknown'
  }))

  const recentApplicants = [
    // This would come from an applications API
    {
      id: 1,
      name: 'Ahmed Rahman',
      position: jobs[0]?.title || 'React Developer',
      experience: '5 years',
    ...
    },
    {
      id: 2,
      name: 'Fatima Khan',
      position: jobs[1]?.title || 'UI/UX Designer',
      experience: '3 years',
    ...
    },
    {
      id: 3,
      name: 'Mohammad Ali',
      position: jobs[2]?.title || 'DevOps Engineer',
      experience: '4 years',
                    ...
                    <CardContent className="p-6">
                      <div className="flex justify-between items-start">
                        ...
                        <div className="flex items-center gap-2">
                          <Badge 
                            variant={job.approved_by_admin ? 'default' : 'secondary'}
                            className={
                              job.approved_by_admin 
                                ? job.is_active 
                                  ? 'bg-green-600' 
                                  : 'bg-gray-600'
                                : 'bg-yellow-600'
            ...
            <Avatar className="h-10 w-10">
              ...
              <AvatarFallback className="bg-yellow-500 text-gray-900 font-semibold">
                {user?.company_name ? user.company_name.substring(0, 2).toUpperCase() : 
                 user?.first_name ? user.first_name.substring(0, 1).toUpperCase() : 'U'}
              </AvatarFallback>
            </Avatar>
            <div className="flex-1 min-w-0">
              <p className="text-sm font-semibold text-white truncate">
                {user?.company_name || `${user?.first_name || ''} ${user?.last_name || ''}`.trim() || 'User'}
              </p>
