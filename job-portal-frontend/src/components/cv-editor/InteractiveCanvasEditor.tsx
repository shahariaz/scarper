...
const CanvasCVEditor: React.FC<CanvasCVEditorProps> = ({ 
    ...
    if (cvData.sections) {
      ...
      cvData.sections.forEach((section) => {
        ...
        section.items.forEach((item, index) => {
          const itemText = new fabric.Textbox(item, {
            left: 50,
  ...
  }, [canvasRef, canvas, showGrid, cvData, template]);
  ...
  const updateSelectedObjectProperty = (property: string, value: unknown) => {
  ...
  const exportToPDF = () => {
    ...
    const gridObjects = canvas.getObjects().filter((obj: any) => obj.excludeFromExport);
                      ...
                      <>
                        <div>
                          ...
                          <Slider
                            ...
                            onValueChange={([value]: number[]) => updateSelectedObjectProperty('fontSize', value)}
                            max={72}
                            min={8}
                            step={1}
                            className="mt-2"
